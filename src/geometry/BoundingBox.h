...
public:
	 ...
	 * @param newPoint this bb will be extended to contain this point (may not be necessary)
	 */
	void extend(const glm::vec3& newPoint);

	/**
	 * Extends this bb.
	 * The resulting BoundingBox is the BoundingBox of the union of boundingBox and this bb.
	 * @param boundingBox the current bb will be extended to contain this bb.
	 */
	void extend(const BoundingBox& boundingBox);

	/**
	 * A valid bb is one for which it makes sense to get the center or the extent.
	 * To be valid, every operation performed on the vertices of the bb must
	 * result in a value different from nan.
