...
std::vector<glm::mat4> SkeletalAnimation::getAt(float ticks, const std::vector<Bone>& skeleton) const {
    ...
    for (std::size_t boneIndex = 0; boneIndex < skeleton.size(); boneIndex++) {
        ...
        if (boneIndex < mBoneKeyFrames.size()) {  // go on only if the key frame is available
            ...
            // positions
            const auto nextPositionIt
                = std::lower_bound(boneKey.positionKeys.begin(), boneKey.positionKeys.end(), ticks);
            const auto positionIndex = std::distance(boneKey.positionKeys.begin(), nextPositionIt);
            if (positionIndex == 0)
                position = originalBone.position;  // default position;
            else if (positionIndex == boneKey.positionKeys.size())
                position = boneKey.positions[positionIndex - 1];
            else {
            ...
            if (scalingIndex == 0)
                scale = originalBone.scale;  // default scale;
            else if (scalingIndex == boneKey.scalingKeys.size())
                scale = boneKey.scalings[scalingIndex - 1];
            else {
            ...
            // rotation
            const auto nextRotationIt
                = std::lower_bound(boneKey.rotationKeys.begin(), boneKey.rotationKeys.end(), ticks);
            const auto rotationIndex = std::distance(boneKey.rotationKeys.begin(), nextRotationIt);
            if (rotationIndex == 0)
                rotation = originalBone.rotation;  // default rotation;
            else if (rotationIndex == boneKey.rotationKeys.size())
                rotation = boneKey.rotations[rotationIndex - 1];
            else {
