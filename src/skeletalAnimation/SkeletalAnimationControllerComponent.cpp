...
bool SkeletalAnimationControllerComponent::isCurrentAnimationStopped() const
{
	...
	return anim != nullptr && (!mTime.isStarted() || 
		(anim->loopDirection == SkeletalAnimation::LoopDirection::STOP && mTime.getSeconds() > anim->getDuration()));
}
...
float SkeletalAnimationControllerComponent::getCurrentAnimationTime() const
{
	...
	if (loopDirection == SkeletalAnimation::LoopDirection::BOUNCE) {
		ticks = (static_cast<int>(ticks / duration) % 2 == 0 ?
			std::fmod(ticks, duration) : duration - std::fmod(ticks, duration));
	}
