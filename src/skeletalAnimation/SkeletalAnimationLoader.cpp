...
SkeletalAnimation SkeletalAnimationLoader::fromAssimpScene(const aiScene* scene, const std::map<std::string, std::uint32_t>& boneName2index)
{
	...
	if (ticksPerSecond == 0) ticksPerSecond = 24.0f; // default to 24
...
SkeletalAnimation SkeletalAnimationLoader::fromFile(const std::string& path, const std::map<std::string, std::uint32_t>& boneName2index)
{
