...
Light::Light(const GameObjectEH& go, Light::Type lightType)
    : Component{ go }
    , mType{ lightType } { }
...
bool Light::needsShadowUpdate() {
    ...
    if (mShadowMode == ShadowCasterMode::DYNAMIC)
        return true;
    ...
    if (mShadowMode == ShadowCasterMode::AUTO) {
        ...
        bool shouldUpdate = glm::any(glm::notEqual(pos, mOldPosition)) || glm::any(glm::notEqual(rot, mOldRotation));
