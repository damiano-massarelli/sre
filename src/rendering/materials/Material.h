...
/**
 * A Material contains all the properties used to define the appareance of an
 * object */
class Material {
private:
...
public:
    ...
    /**
     * Creates a new material using a vertexShader a fragmentShader and a geom
     * shader. Shaders should be stored on disk.
     * @param vertexShader the path of the vertex shader
    ...
    /**
     * Creates a new material using vertex shaders, fragment shader and a geom
     * shader. Shaders should be stored on disk. The provided fragment shaders
     * are combined together one after the other in the order specified in the
     * vector. The same process is used for the vertex shaders and geometry
     * shader.
    ...
    Material(const std::vector<std::string>& vertexShaders,
     ...
     * their materials should return true. The order in which Mesh%es are
     * rendered is determined by the value of renderOrder()
     * @return true if the corresponding mesh needs to be drawn in a certain
     * order, false otherwise */
    virtual bool needsOrderedRendering() { return false; };

    /**
     * Provides a value to order Mesh%es.
     * @param position the position of the GameObject to which this material
     * belongs
