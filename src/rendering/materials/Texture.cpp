...
Texture Texture::loadFromMemoryCached(
    const std::string& cacheKey, std::uint8_t* data, std::int32_t len, const AppearanceSettings& options) {
...
Texture Texture::loadCubemapFromFile(const std::map<std::string, std::string>& paths) {
    std::map<std::string, int> side2glSide{ { "front", GL_TEXTURE_CUBE_MAP_NEGATIVE_Z },
...
Texture Texture::loadCubemap(const std::map<std::string, void*>& data,
    int width,
    int height,
    int wrapS,
    int wrapT,
...
Texture Texture::load(void* data, int width, int height, const Settings& options) {
    ...
    glTexImage2D(GL_TEXTURE_2D,
        ...
        height,
        0,
        options.dataPixelFormat,
        options.dataPixelType,
        data);
