...
std::size_t PBRMaterial::hash() const
{
	...
	hash_combine(seed, mAlbedoMap.getId(), mNormalMap.getId(), mMetalnessMap.getId(), mRoughnessMap.getId(), mAOMap.getId(), boolMask);
...
bool PBRMaterial::equalsTo(const Material* rhs) const
{
	...
	return Material::equalsTo(rhs)
		&& glm::all(glm::epsilonEqual(mAlbedo, other->mAlbedo, EPS))
		&& glm::epsilonEqual(mMetalness, other->mMetalness, EPS)
		&& glm::epsilonEqual(mRoughness, other->mRoughness, EPS)
		&& glm::epsilonEqual(mAO, other->mAO, EPS)
		&& mAlbedoMap.getId() == other->mAlbedoMap.getId()
		&& mNormalMap.getId() == other->mNormalMap.getId()
		&& mMetalnessMap.getId() == other->mMetalnessMap.getId()
		&& mRoughnessMap.getId() == other->mRoughnessMap.getId()
		&& mAOMap.getId() == other->mAOMap.getId()
		&& mUseAlbedoMap == other->mUseAlbedoMap
