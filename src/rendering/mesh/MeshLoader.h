...
public:
    ...
    static Mesh createMesh(const float vertexData[],
        std::uint32_t numOfVertices,
        std::uint32_t indices[],
        std::uint32_t numOfIndices,
     ...
     * @param dataPerVertex how many elements of the array are describe a single
     * vertex
     * @param bufferType the type of buffer to create (GL_ARRAY_BUFFER,
     * GL_ELEMENT_ARAY_BUFFER, etc)
     * @param dataType the type of the data to load (GL_FLOAT, GL_UNSIGNED_INT,
     * etc).
     * @param addToAttribPointer if true an attrib pointer is created for the
     * data provided
     * @param usage the usage (static, stream, etc) */
    template <typename T>
    GLuint loadData(const T* data,
        std::size_t size,
        GLint dataPerVertex,
        GLenum bufferType = GL_ARRAY_BUFFER,
        GLenum dataType = GL_FLOAT,
        bool addToAttribPointer = true,
        ...
        if (addToAttribPointer) {
            ...
            if constexpr (std::is_floating_point_v<T>) {
                glVertexAttribPointer(
                    mCurrentAttribPointer, dataPerVertex, dataType, GL_FALSE, dataPerVertex * sizeof(T), (void*)0);
            } else if constexpr (std::is_integral_v<T>) {
                glVertexAttribIPointer(
                    mCurrentAttribPointer, dataPerVertex, dataType, dataPerVertex * sizeof(T), (void*)0);
            }
    ...
    GLuint addAttribPointer(
        GLenum bufferType, GLuint vbo, GLsizei stride, GLint dataPerVertex, GLenum dataType, std::uintptr_t offset) {
     ...
     * @param the number of indices. If this value is different from 0 the mesh
     * will use indexed rendering.
