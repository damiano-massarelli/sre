...
void ShadowOnVisibleSceneComponent::computeWidthsAndHeights() {
    /*float aspectRatio = Engine::renderSys.getScreenWidth() /
    (float)Engine::renderSys.getScreenHeight();

    float fovy = Engine::renderSys.getVerticalFov() / 2;
    nearHeight = std::tan(fovy) * Engine::renderSys.getNearPlane();
    farHeight = std::tan(fovy) * Engine::renderSys.getFarPlane() *
    shadowDistance;
...
void ShadowOnVisibleSceneComponent::onEvent(SDL_Event e) {
    ...
    glm::vec3 toFar = cameraTransform.forward() * renderSys.getFarPlane() *
    shadowDistance;

    glm::vec3 centerNear = toNear + cameraTransform.getPosition();
    glm::vec3 centerFar = toFar + cameraTransform.getPosition();

    auto verices = calculateFrustumVertices(cameraTransform.up(),
    cameraTransform.right(), centerNear, centerFar);
    ...
    Engine::renderSys.getViewMatrix(gameObject->transform) };
    gameObject->transform.setPosition(glm::inverse(view) * center);*/
}

std::array<glm::vec3, 8> ShadowOnVisibleSceneComponent::calculateFrustumVertices(
    ...
    std::array<glm::vec3, 8> points{ centerFar + toFarTopRight,
