...
void RenderSystem::initGL(std::uint32_t width, std::uint32_t height) {
    ...
    // 3 matrices: view, projection, projection * view and a vec4 for the
    // clipping plane
...
void RenderSystem::updateLights() {
    ...
    for (std::size_t i = 0; i < numLight; i++) {
        ...
        if (castShadow) {
            ...
            // 			lightProjection[0][0] = 2.0f /
            // shadowMappingSettings.width; 			lightProjection[1][1]
            // = 2.0f / shadowMappingSettings.height;
...
glm::mat4 RenderSystem::getViewMatrix(const Transform& transform) {
    ...
    /* Inverts the view so that we watch in the direction of
