...
template <> struct hash<Material*> {
    std::size_t operator()(const Material* material) const { return material->hash(); }
};

template <> struct equal_to<Material*> {
    bool operator()(const Material* lhs, const Material* rhs) const { return lhs->equalsTo(rhs); }
};
