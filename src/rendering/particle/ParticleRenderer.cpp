...
template <class Iterator, class Compare> void insertionSort(Iterator begin, Iterator end, Compare cmp) {
    auto current = begin;
    while (current != end) {
...
void ParticleRenderer::init() {
    ...
    mParticleShader = Shader::loadFromFile(
        std::vector<std::string>{ "shaders/particleVS.glsl" }, {}, { "shaders/particleFS.glsl" });
...
void ParticleRenderer::prepareParticleQuad() {
    ...
    mParticleDataVBO = loader.loadData<float>(
        nullptr, MAX_PARTICLES * FLOATS_PER_PARTICLE, 0, GL_ARRAY_BUFFER, GL_FLOAT, false, GL_STREAM_DRAW);
...
void ParticleRenderer::storeOffsetsAndBlendFactor(
    const Particle& p, const ParticleEmitter* emitter, std::vector<float>& data) {
