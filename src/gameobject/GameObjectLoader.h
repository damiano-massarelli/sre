...
/**
 * A GameObjectLoader is used to create a GameObject form a model stored on a
 * file. */
...
private:
    ...
    /** used to filter channels from one texture */
    Shader mTextureChannelFilter;
    ...
    /** Extracts the metalness and the roughness texture from a single
     * metalnessRoughness Texture*/
    void setMetalnessRoughnessTexture(std::shared_ptr<PBRMaterial> material, const Texture& metalnessRoughness);
    ...
    void computeTangentsAndBitangentSign(
...
public:
     ...
     * If the loaded model has bones a SkeletalAnimationControllerComponent is
     * added as its component. If the model also contains a skeletal animation
     * it is added to the SkeletalAnimationControllerComponent and its name is
     * "default"
     * (@see SkeletalAnimationControllerComponent::playAnimation )
     * @param path the path of the model to load
     * @return a reference to the root GameObject (invalid reference if the
