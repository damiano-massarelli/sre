...
GameObjectEH GameObjectLoader::processNode(aiNode* node, const aiScene* scene) {
    ...
    for (std::uint32_t i = 0; i < node->mNumChildren; ++i) {
         ...
         * When go->transform is evaluated it returns a pointer to an element of
         * a vector, when a new child is added this pointer may become invalid.
         * Hence, using go->transform.addChild(processNode(node->mChildren[i],
         * scene)); causes undefined behavior since go->transform is evaluated
         * before a new game object is added */
        auto child = processNode(node->mChildren[i], scene);
        go->transform.addChild(child);
    }

    /* assimp only provides the transformation matrix relative to the parent
     * node hence we need to extract position, rotation and scale to obtain the
     * local components of that matrix */
    glm::vec3 position, scale;
    glm::quat rotation;
    decompose(convertMatrix(node->mTransformation), position, rotation, scale);

    /* There is no need to use local transformations since when the following
     * code is executed all the children have their position already set
     * relative to the parent (thanks to the decomposed matrix). When the
     * following transformations are applied they also affect the children of
     * this node */
...
void GameObjectLoader::processMesh(
    const GameObjectEH& go, aiNode* node, int meshNumber, aiMesh* mesh, const aiScene* scene) {
    ...
    // warning messages
    if (!mesh->HasNormals())
        std::cout << "Mesh: " << mesh->mName.C_Str() << " node: " << node->mName.C_Str()
                  << ": cannot find normals, setting them to (0, 0, 0)\n";

    if (!mesh->HasTextureCoords(0))
        std::cout << "Mesh: " << mesh->mName.C_Str() << " node: " << node->mName.C_Str()
                  << ": cannot find uv coords, setting them to (0, 0)\n";

    const bool needsTangents = !mesh->HasTangentsAndBitangents();
    const bool shouldHaveTangents
        = ((scene->mMaterials[mesh->mMaterialIndex]->GetTextureCount(aiTextureType_HEIGHT) != 0
    ...
    if (needsTangents && shouldHaveTangents) {
        std::cout << "Mesh: " << mesh->mName.C_Str() << " node: " << node->mName.C_Str()
                  << ": has bump map/parallax map but no tangent data, will be "
                     "calculated\n";
    }
    ...
    for (std::uint32_t i = 0; i < mesh->mNumVertices; ++i) {
        ...
        if (mesh->HasTangentsAndBitangents()) {
            ...
            // TODO compute direction of bitangent as an int (+1 or -1) to pass
            // to shader
        }
    ...
    Mesh loadedMesh
        = loader.getMesh(static_cast<std::uint32_t>(vertices.size()), static_cast<std::uint32_t>(indices.size()));
...
MaterialPtr GameObjectLoader::processMaterial(aiMesh* mesh, const aiScene* scene, const std::string& cacheName) {
    ...
    if (shadingModeHintDefined == aiReturn_FAILURE || shadingMode == aiShadingMode_CookTorrance) {
        ...
        pbrMaterial->setAlbedoMap(
            loadTexture(material, scene, AI_MATKEY_GLTF_PBRMETALLICROUGHNESS_BASE_COLOR_TEXTURE, cacheName));
        pbrMaterial->setAmbientOcclusionMap(loadTexture(material,
            scene,
            aiTextureType_LIGHTMAP,
            0,
            cacheName));  // In assimp AO texture is called lightmap, ok...
        setMetalnessRoughnessTexture(pbrMaterial,
            loadTexture(material, scene, AI_MATKEY_GLTF_PBRMETALLICROUGHNESS_METALLICROUGHNESS_TEXTURE, cacheName));
...
Texture GameObjectLoader::loadTexture(
    aiMaterial* material, const aiScene* scene, aiTextureType type, int id, const std::string& meshCacheName) {
    ...
    if (material->GetTextureCount(type) != 0) {
        ...
        if (const aiTexture* texture = scene->GetEmbeddedTexture(texturePath)) {
            ...
            if (texture->mHeight == 0)
                return Texture::loadFromMemoryCached(
                    meshCacheName + std::string{ texturePath }, textureData, texture->mWidth, loadOptions);
            else
                return Texture::loadFromMemoryCached(meshCacheName + std::string{ texturePath },
                    textureData,
...
void GameObjectLoader::setMetalnessRoughnessTexture(
    std::shared_ptr<PBRMaterial> material, const Texture& metalnessRoughness) {
    if (metalnessRoughness.isValid()) {
        ...
        Texture roughnessTexture
            = Texture::load(nullptr, metalnessRoughness.getWidth(), metalnessRoughness.getHeight(), textureSettings);
        renderTarget = RenderTarget{ &roughnessTexture, nullptr };
...
void GameObjectLoader::findBones(const aiScene* scene) {
    for (unsigned int i = 0; i < scene->mNumMeshes; ++i) {
        ...
        for (unsigned int j = 0; j < mesh->mNumBones; j++) {
            ...
            // multiple meshes may reference the same bone, add it only the
...
void GameObjectLoader::computeTangentsAndBitangentSign(
    const aiMesh* mesh, std::vector<float>& tangents, std::vector<std::int32_t> bitangentSign) const {
    if (!mesh->HasTextureCoords(0)) {
        tangents.resize(3 * static_cast<std::size_t>(mesh->mNumVertices), 0.0f);
        bitangentSign.resize(mesh->mNumVertices, 0);
        std::cerr << "Cannot calculate tangent space: texture coordinates are "
                     "missing\n";
        return;
    }
...
std::vector<std::uint32_t>& GameObjectLoader::getInfluencingBones(std::uint32_t vertexIndex, aiMesh* mesh) {
    ...
    if (bones.size() > 4)
        std::cerr << "more than 4 bones per vertex, not supported\n";
...
GameObjectLoader::GameObjectLoader() {
    mTextureChannelFilter = Shader::loadFromFile(
        { "effects/genericEffectVS.glsl" }, {}, std::vector<std::string>{ "effects/textureChannelExtractorFS.glsl" });
}
...
GameObjectEH GameObjectLoader::fromFile(const std::string& path) {
    ...
    if (mBones.size() != 0) {
        mSkeletalAnimationController
            = std::make_shared<SkeletalAnimationControllerComponent>(GameObjectEH{}, mBones, mBoneName2Index);
