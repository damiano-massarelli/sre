...
GameObjectEH::GameObjectEH(HandleList<GameObject>* handleList, std::uint32_t index, std::uint32_t generation) : ExternalHandle{index, generation}, mHandleList{handleList} {
     ...
     * To do so they need an external handle which is created after the game object itself.
     * So the external handle must take care of this. */
    if (isValid())
        mHandleList->get(mHandleIndex, mGeneration).transform.gameObject = *this;
}


GameObjectEH::GameObjectEH() : GameObjectEH{nullptr, 0, 0} {}
