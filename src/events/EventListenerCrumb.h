...
/**
 * Event listener crumbs automatically unregister an event listener when it is
 * deleted. The EventListener receiving it should store it in a member variable
 * so that when the EventListener is destroyed the EventListenerCrumb can
 * automatically unregister it.
 */
...
public:
     ...
     * crumb for an event X and it has to be attached to event Y using the same
     * crumb
