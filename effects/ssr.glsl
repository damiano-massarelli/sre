...
vec4 ssr(vec4 color) {
	...
	vec3 rayEnd = rayStart + (rayDirection * _ssr_rayMaxDistance);
	...
	for (float i = 0; i < int(delta); ++i) {
		// Discard fragments out of the screen
		...
		if (depthDelta > 0 && depthDelta < _ssr_rayHitThreshold) {
			hit = 1;
			break;
		} else {
			search0 = search1;
	...
	vec2 uvOutOfBounds = 1.0 - smoothstep(vec2(0.85), vec2(1.0), uv);
	...
	float furtherFromHit = (1.0 - clamp(depthDelta / _ssr_rayHitThreshold, 0.0, 1.0));
