...
float _ssr_viewSpaceDepthAt(vec2 coord) {
    float z = texture(_ssr_depthBuffer, coord).r;
	return _ssr_toLinearDepth(z);
}

float _ssr_getAttenuation(
	RayHitInfo hitInfo,
	vec3 wsRayStart,
	vec3 wsHitPoint,
	float csRayEndDepth,
	...
	float furtherFromFirstRefl = 1.0 - hitDistanceFromStart / rayMaxDistance;

	return hitInfo.hit * furtherFromHit * furtherFromFirstRefl \
...
vec4 ssr() {
	...
	// Ray might be shorter, recompute distance
	float rayMaxDistance = distance(wsRayStart, wsRayEnd);
	...
	if (_ssr_useFallbackSkybox) {
		baseColor = texture(_ssr_fallbackSkybox, wsRayDirection).rgb;
	}

	if (result.hit == 0) {
		return vec4(baseColor * fresnel, roughness);
	}
	...
	const vec3 wsHitPoint = wsRayStart + wsRayDirection * rayMaxDistance * result.distanceTraveledPercentage;
	const float attenuation = _ssr_getAttenuation(result, wsRayStart, wsHitPoint,
															csRayEndDepth, biasedThreshold, rayMaxDistance);
	vec3 reflectedColor = texelFetch(_ssr_sceneTexture, ivec2(textureSize(_ssr_sceneTexture, 0) * result.uvHitPoint), 0).rgb;

	reflectedColor = mix(baseColor, reflectedColor, attenuation) * fresnel;

	return vec4(reflectedColor, roughness);
}
