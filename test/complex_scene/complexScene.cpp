...
static void addParticles(const GameObjectEH& eh) {
    ...
    // eh->getComponent<ParticleEmitter>()->setTextureAtlas(Texture::loadFromFile("test_data/particle/fire.png"),
    // 25, 5, 5); eh->getComponent<ParticleEmitter>()->start(300.0f);
...
void ComplexTestScene::start() {
    ...
    // auto water =
    // Engine::gameObjectManager.createGameObject(MeshCreator::plane(),
    ...
    // water->transform.rotateBy(glm::angleAxis(glm::radians(-90.0f),
    // water->transform.right())); water->transform.scaleBy(glm::vec3{ 290.0f,
    // 140.0f, 1.0f });
